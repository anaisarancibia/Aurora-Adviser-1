...
class Caso {
  ...
  display() {
    ...
    let singleContainer = createDiv(
      "<div class='desc'>" +this.name + this.desc + "<br><br><strong>Palabras Clave</strong>: " + this.keywords + "<br><br><a href='" + this.casiopeaURL + "'></a></div>" +
      "<img src='" + this.img + "' title='" + this.name + "' />"
    );
...
function constructSelector() {
  ...
  // Crear un set para almacenar keywords únicas
  let uniqueKeywords = new Set();

  // Recorrer los casos y agregar las keywords permitidas al set
  casos.forEach(caso => {
    caso.keywords.split(', ').forEach(keyword => {
      if (keywordsPermitidos.includes(keyword) && !uniqueKeywords.has(keyword)) {
  ...
  sel.changed(() => {
    let selectedKeyword = sel.value();
    displayCasesByKeyword(selectedKeyword);
  });
}
...
// Función para mostrar casos que contienen una keyword específica.
function displayCasesByKeyword(keyword) {
  clearAll();
  let htmlTitle = document.getElementById('title');
  htmlTitle.innerHTML = "Casos con la palabra clave: " + keyword;
  for (let caso of casos) {
    if (caso.keywords.includes(keyword)) {
      caso.displaySmall();
    }
  }
}
